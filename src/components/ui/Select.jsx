...
export const Select = ({
  ...
  return (
    <SelectChevron
      vect={vect}
      icon={(multiIcon && vect[0]?.icone) || Icon}
      onChange={onChange}
      multiIcon={multiIcon}
      value={value}
...
const SelectChevron = ({
  ...
  const lastAutoSelectedValue = useRef();

  const normalizedVect = useMemo(
    () =>
      vect
        .filter(Boolean)
        .map((item) =>
          typeof item === "string"
            ? { value: item, label: item, icone: item }
            : item
        ),
    [vect]
  );
  ...
  const findSelectedItem = (val) => {
    if (val !== undefined && val !== null && val !== "") {
      const selectedItem = normalizedVect.find(
  ...
  useEffect(() => {
    ...
    if (!hasExplicitValue && newSelected?.value !== undefined && newSelected.value !== "") {
      const autoValue = String(newSelected.value);

      if (lastAutoSelectedValue.current !== autoValue) {
        lastAutoSelectedValue.current = autoValue;
        onChange?.(newSelected.value);
      }
    }

    if (hasExplicitValue) {
      lastAutoSelectedValue.current = undefined;
    }
  }, [value, normalizedVect, onChange, selected?.value]);
        ...
        >
          ...
          >
            {normalizedVect.map((item, i) => (
              <li key={item.value ?? i} onClick={blockEvent}>
                <button
                  type="button"
